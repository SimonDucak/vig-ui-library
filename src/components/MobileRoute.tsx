...
export const MobileRoute = ({ children, onClose, ...restProps }: MobileRouteProps) => {
    ...
    return (
        <MobileRouteWrapper open={open}>
            <MobileRouteHeader onClose={beforeClose} {...restProps}></MobileRouteHeader>
...
export type MobileRouteHeaderProps = Omit<MobileRouteProps, "children">

export const MobileRouteHeader = ({ title, onClose }: MobileRouteHeaderProps) => {
    const theme = useTheme();

    return (
        <Box display="flex" justifyContent="space-between" width="100%">
            <Box
                display="flex" alignItems="center" p={2} borderBottom={1}
                borderColor={theme.palette.divider} flexGrow={1} gap={1}
            >
                <IconButton
                    color="primary"
